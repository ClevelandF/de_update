    ...
    sidebarLayout(
      sidebarPanel(
        ...
        conditionalPanel(
          ...
          tooltip(
            bsicons::bs_icon("question-circle"),
            "Family is in a subgroup when at least one individual in the household is: of selected age | in slected family type | receiving selected benefit",
            placement = "right"
          ),
          pickerInput(
...
server <- function(input, output, session) {
  ...
  output$ventilePlot = renderPlotly({
    ...
    p = ggplot(ventiles, aes(x = as.numeric(Income.Group), y = Population, fill = Description)) +
      labs(title = paste0(str_to_title(paste("Population Distribution By", input$populationType, input$incomeSort, "Ventiles", sep = " ")), "\nTax Year 20", data_year(), " from ", data_hes(), " and ", data_efu()),
  ...
  output$incomeBandPlot = renderPlotly({
    ...
    p = ggplot(incomeBand, aes(x = Band, y=Population, fill=Description)) +
      labs(title = paste0(str_to_title(paste("Population Distribution By", input$populationType, input$incomeSort, "Bands", sep = " ")), "\nTax Year 20", data_year(), " from ", data_hes(), " and ", data_efu()),
  ...
  output$ventileDataTitle = renderText({
    str_to_title(paste("Population Distribution By", input$populationType, input$incomeSort, "Ventiles", sep = " "))
  })
  ...
  output$incomeBandDataTitle = renderText({
    str_to_title(paste("Population Distribution By", input$populationType, input$incomeSort, "Bands", sep = " "))
  })
